...
impl<'a> Executor<'a> {
    ...
    pub fn execute(&mut self) -> Result<bool, ExecutionError> {
        ...
        println!(
            "program.pc_base, program.pc_start in `execute`: {}, {}",
            program.pc_base, program.pc_start
        );
        ...
        for (i, record) in self.records.iter_mut().enumerate() {
            ...
            if record.cpu_events.is_empty() {
                ...
                record.public_values.next_pc = last_next_pc % 2013265921;
                ...
                record.public_values.next_pc =
                    record.cpu_events.last().unwrap().next_pc % 2013265921;
